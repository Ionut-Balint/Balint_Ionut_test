...
void valid(int st[100] , int k , int a[100][100] , int n , int &ev)
{
    ...
    if(ev)
        for(int i=1;i<=k-2;i++)
            if((st[k]==st[i+1] && st[k-1]==st[i]) || (st[k]==st[i] && st[k-1]==st[i+1]))
                ev=0;
}
...
int solutie(int st[100] , int k , int s)
{
    return(st[k]==s);
}

void back_tracking(int a[100][100] , int n , int p , int s)
{
